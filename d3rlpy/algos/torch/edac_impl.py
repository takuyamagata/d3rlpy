...
class EDACImpl(SACImpl):
    ...
    # override .ddpg_impl::DDPGBaseImpl
    def compute_critic_loss(
            self, batch: TorchMiniBatch, q_tpn: torch.Tensor
        ) -> torch.Tensor:
            assert self._q_func is not None
            q_loss = self._q_func.compute_error(
            ...
            es_loss = self._compute_ensemble_similarity_metric(batch)
            
            return q_loss + self._eta * es_loss
            
    def _compute_ensemble_similarity_metric(
        self, batch: TorchMiniBatch
        ) -> torch.Tensor:
        r""" Compute Ensemble Similarity metric
        """       
        observations = batch.observations.unsqueeze(0).repeat(self._n_critics,1,1)
        actions = batch.actions.unsqueeze(0).repeat(self._n_critics,1,1).requires_grad_(True)
        q_values = self._q_func(
                observations,
                actions,
                reduction="none",
                tile=True,
            )
        dqda = torch.autograd.grad(q_values.sum(), actions, retain_graph=True, create_graph=True)[0]
        dqda = dqda / (torch.norm(dqda, p=2, dim=2).unsqueeze(-1) + 1e-10)
        dqda = dqda.transpose(0,1) # <ensemble_models, batch, action> -> <batch, ensemble_models, action>
        es_loss = torch.einsum('bik,bjk->bij', dqda, dqda)
        masks = torch.eye(self._n_critics, device=self.device).unsqueeze(dim=0).repeat(batch.actions.shape[0], 1, 1)
        es_loss = (1-masks) * es_loss
        es_loss = torch.mean(torch.sum(es_loss, dim=(1,2))) / (self._n_critics - 1)
        return es_loss
        
class DiscreteEDACImpl(DiscreteQFunctionMixin, TorchImplBase):
