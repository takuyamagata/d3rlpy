...
class TedQFunction(nn.Module):  # type: ignore
    _action_size: int
    ...
    def compute_error(
        ...
        for n, q_func in enumerate(self._q_funcs):
...
class TedContinuousQFunction(TedQFunction):
    def forward(
        self, x: torch.Tensor, 
        action: torch.Tensor, 
        tile: bool = False,
    ...
    def __call__(
        ...
        return cast(torch.Tensor, super().__call__(x, action, tile))
